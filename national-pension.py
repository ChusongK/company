...
class PensionData():
    def __init__(self, filepath):
        self.df = pd.read_csv(os.path.join(filepath), encoding='cp949',error_bad_lines=False)
        self.pattern1 = '(\([^)]+\))'
        self.pattern2 = '(\[[^)]+\])'
...
def read_pensiondata():
    data = PensionData('https://www.dropbox.com/scl/fi/4t2guzz346d8j92b9fwrm/national-pension.csv?rlkey=da3e6ur1sxyo1p5eq763v0nqw&dl=0')
    return data
